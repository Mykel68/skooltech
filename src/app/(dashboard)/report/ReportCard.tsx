        ...
        <div ref={reportRef} className="p-12 print-area">
          ...
          <div className="flex items-center justify-between border-b border-gray-300 pb-4 mb-6">
            ...
            <div className="flex-1 px-4 text-center max-w-2xl">
              ...
              <p className="text-sm text-balance">{schoolInfo.address}</p>
              <p className="text-sm">
                Tel: {schoolInfo.phone_number} | Email: {userEmail}
              </p>
              <p className="italic text-sm mt-1">"{schoolInfo.motto}"</p>
            ...
            <div className="flex-shrink-0 pl-4 ml-4">
              <div className="w-24 aspect-square bg-primary text-white rounded flex flex-col items-center justify-center text-center">
                ...
                <div className="p-1 font-semibold text-sm">{student.term}</div>
              </div>
            </div>
          </div>
              ...
              <table className="w-full border border-black mb-4">
                <tbody>
                  ...
                  <tr>
                    ...
                    <td className="border p-1">
                      <strong className="text-sm">{student.lastName}</strong>{" "}
                    </td>
                  </tr>
                  ...
                  <tr>
                    ...
                    <td className="border p-1">
                      <strong className="text-sm">{student.class}</strong>{" "}
                    </td>
                  </tr>
                  <tr>
                    <td className="border p-1">
                    ...
                    <td className="border p-1">
                      <strong className="text-sm">{student.gender}</strong>{" "}
                    </td>
                  </tr>
                </tbody>
              </table>
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td className="border p-1 text-center">
                        {student.attendance?.totalDays ?? "N/A"}
                      </td>
                      <td className="border p-1 text-center">
              ...
              <div className="space-y-1">
                ...
                <table className="w-full border border-black mb-4">
                  <tbody>
                    ...
                    <tr>
                      <td className="border p-1 text-center">
                        {formatDate(student.termStarts) ?? "N/A"}
                      </td>
                      <td className="border p-1 text-center">
                        {formatDate(student.termEnds) ?? "N/A"}
                      </td>
                      <td className="border p-1 text-center">
                        {formatDate(student.nextTermBegins) ?? "N/A"}
                      </td>
                    </tr>
                  </tbody>
                </table>
          ...
          <table className="w-full border border-black mb-4">
            <thead className="bg-gray-100">
              <tr>
                ...
                {componentNames.map((n) => (
                  <th key={n} className="border p-1 text-sm">
                    {n}
                  </th>
                ))}
            ...
            <tbody>
              {student.subjects.map((subj) => (
                <tr key={subj.name}>
                  <td className="border p-1 text-sm">{subj.name}</td>
                  {componentNames.map((cn) => {
                    const comp = subj.components.find(
                      (c) => c.component_name === cn
                    );
                    return (
                      <td key={cn} className="border p-1 text-center">
                        {comp?.score ?? "-"}
                      </td>
                    );
                  })}
                  ...
                  <td className="border p-1 text-center">{subj.grade}</td>
                  <td className="border p-1">{subj.remark}</td>
                  <td className="border p-1 text-center">
                    {subj.position}
          ...
          <table className="w-full border border-black mb-4 text-sm">
            <tbody>
              <tr>
                ...
                <td className="border p-2">
                  <strong>POSITION IN CLASS:</strong> {student.position}
              ...
              <tr>
                ...
                <td className="border p-2">
                  <strong>AVERAGE:</strong> {student.average.toFixed(1)}%
                </td>
              </tr>
            </tbody>
          </table>
