...
export default function ClassSetupPage() {
	...
	const { data: fetchedClasses, isLoading } = useQuery<any[], Error>({
	...
	useEffect(() => {
		if (fetchedClasses) {
			setExistingClasses(fetchedClasses.map((cls) => cls.grade_level));
		}
	}, [fetchedClasses]);
	...
	const existingFetchedClasses = (fetchedClasses?.length ?? 0) > 0;
